...
def generate_audio(
    text: str, voice_name: str, voice_file: str, voice_rate: float = 1.0, voice_pitch: float = 1.0, proxy: str = None
) -> Union[SubMaker, None]:
    ...
    try:
        ...
        if len(sub_items) == len(script_lines):
            with open(subtitle_file, "w", encoding="utf-8") as file:
                file.write("\n".join(sub_items) + "\n")
            try:
                # Parse duration from last subtitle item
                duration = 0.0
                if sub_items:
                    ...
                    if len(lines) >= 2:
                        ...
                        if '-->' in time_line:
                            ...
                            # Replace comma with dot for parsing if needed, but utils.time_to_seconds handles comma
                            duration = utils.time_to_seconds(end_time_str)
                ...
                logger.error(f"failed, error: {str(e)}")
                # os.remove(subtitle_file) # Don't remove
