...
class AI302Provider(AIProviderBase):
    """302AI平台模型提供商"""

    def __init__(self, config: AIModelConfig):
        if not config.base_url:
            config.base_url = "https://api.302ai.cn/v1/chat/completions"
        else:
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                if isinstance(err, dict):
                    raise Exception(f"302AI API错误: {err.get('message') or response.text}")
                else:
                    raise Exception(f"302AI API请求失败: {response.text}")

            if "error" in response_data:
                error_info = response_data["error"]
                raise Exception(f"302AI API错误: {error_info.get('message', 'Unknown error')}")

            return response_data
        except Exception as e:
            logger.error(f"302AI API请求失败: {e}")
            raise
