...
"""
文案生成模型配置管理模块
支持动态配置文案生成AI模型参数，包括提供商、API密钥、模型名称等
"""
...
class ContentModelConfigManager:
    """文案生成模型配置管理器"""
    ...
    def __init__(self, config_file: Optional[str] = None):
        ...
        Args:
            config_file: 配置文件路径，默认为 backend/config/content_model_config.json
        """
        if config_file is None:
            ...
            config_file = config_dir / "content_model_config.json"
        
        self.config_file = Path(config_file)
        self.configs: Dict[str, ContentModelConfig] = {}
    ...
    def save_configs(self):
        ...
        try:
            ...
            logger.info("文案生成模型配置保存成功")
            
        except Exception as e:
            logger.error(f"保存文案生成模型配置失败: {e}")
            raise
    ...
    def add_config(self, config_id: str, config: ContentModelConfig) -> bool:
        """
        添加新配置
        ...
        try:
            ...
            logger.info(f"添加文案生成模型配置成功: {config_id}")
            return True
            
        except Exception as e:
            logger.error(f"添加文案生成模型配置失败: {e}")
            return False
    ...
    def delete_config(self, config_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"删除文案生成模型配置成功: {config_id}")
            return True
            
        except Exception as e:
            logger.error(f"删除文案生成模型配置失败: {e}")
            return False
    ...
    def get_config(self, config_id: str) -> Optional[ContentModelConfig]:
        ...
        Returns:
            ContentModelConfig: 配置对象，如果不存在返回None
        """
        return self.configs.get(config_id)

    def get_all_configs(self) -> Dict[str, ContentModelConfig]:
        """获取所有配置"""
        return self.configs.copy()
    
    def get_enabled_configs(self) -> Dict[str, ContentModelConfig]:
        """获取所有启用的配置"""
        return {
            config_id: config 
            for config_id, config in self.configs.items() 
    ...
    def set_active_config(self, config_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"设置活跃文案生成模型配置: {config_id}")
            return True
            
        except Exception as e:
            logger.error(f"设置活跃文案生成模型配置失败: {e}")
            return False

    def get_active_config(self) -> Optional[ContentModelConfig]:
        """获取当前活跃配置"""
...
# 全局文案生成模型配置管理器实例
content_model_config_manager = ContentModelConfigManager()