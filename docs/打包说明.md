**打包与发布说明（Windows 版｜Tauri + PyInstaller）**
...
- 在项目根目录打开 PowerShell：
  ...
  - 脚本将优先使用 `cnpm`（若不可用回退 `npm`），按顺序完成：前端构建 → 后端打包 → 复制后端到资源目录 → Tauri 构建
...
- `src-tauri/tauri.conf.json`：
  ...
  - `bundle.resources`: `["resources/superAutoCutVideoBackend.exe"]`
  - 当前仅启用 NSIS 安装包：`bundle.targets: ["nsis"]`

**CI 打包（GitHub Actions）**
- 工作流文件：`.github/workflows/build-windows.yml`（仅 Windows）
- 触发条件：
  - push 到 `master` / `main`
  - 创建标签 `v*`（例如 `v1.0.0`）
  - 手动触发 `workflow_dispatch`
- 主要步骤：
  - 安装 Node / Python / Rust / Tauri CLI / FFmpeg / NSIS
  - 前端：`npm ci`
  - 后端：`pip install -r requirements.txt` + `pyinstaller --onefile`
  - 复制后端至 `src-tauri/resources`
  - 修补 `tauri.conf.json` 中的 `cnpm` → `npm`（确保 CI 不依赖 cnpm）
  - 构建：`cargo tauri build --config tauri.conf.json`
- 构建产物：
  - NSIS 安装包：`src-tauri/target/release/bundle/nsis/*.exe`
  - 以 `windows-nsis` 作为 artifact 上传；打标签会自动发布到 Release
...
**手动打包（进阶/排障，仅 Windows）**
- 前端：
  - `cd frontend && cnpm install || npm install && npm run build && cd ..`
- 后端（PyInstaller）：
  - `cd backend`
  ...
  - `cd ..`
- 复制后端到资源目录：
  - `mkdir src-tauri\resources && copy backend\dist\superAutoCutVideoBackend.exe src-tauri\resources\`
- Tauri 打包：
