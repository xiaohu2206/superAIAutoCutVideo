...
import { Check, Layers, Loader, Pause, Pencil, Play, Trash2, Wand2 } from "lucide-react";
import React, { useMemo, useState } from "react";
import type { VoxcpmCloneEvent } from "../hooks/useVoxcpmVoices";
import type { VoxcpmTtsVoice } from "../types";
import VoxcpmCloneProgressItem from "./VoxcpmCloneProgressItem";
import { createPortal } from "react-dom";

export type VoxcpmVoiceListProps = {
  voices: VoxcpmTtsVoice[];
  cloneEventByVoiceId: Record<string, VoxcpmCloneEvent>;
  activeVoiceId: string;
  configId: string | null;
  provider: string;
  onSetActive: (voiceId: string) => Promise<void> | void;
  onStartClone: (voiceId: string) => Promise<void> | void;
  onEdit: (voice: VoxcpmTtsVoice) => void;
...
const KindBadge = () => {
    return (
        <span className={`text-[10px] px-1.5 py-0.5 rounded border flex items-center gap-1 bg-blue-50 border-blue-200`}>
            <Layers className="w-3.5 h-3.5 text-blue-600" />
            <span className="text-blue-700">克隆</span>
        </span>
    )
}

export const VoxcpmVoiceList: React.FC<VoxcpmVoiceListProps> = ({
  voices,
  cloneEventByVoiceId,
  activeVoiceId,
  configId,
  ...
  const [deleteDialog, setDeleteDialog] = useState<{ open: boolean; voice: VoxcpmTtsVoice | null }>({
    open: false,
    voice: null,
  });
  ...
  const handlePreview = async (voice: VoxcpmTtsVoice) => {
  ...
  const openDelete = (v: VoxcpmTtsVoice) => {
  ...
  return (
    <div className="space-y-2">
      {sorted.map((v) => {
        ...
        return (
          <div key={v.id} className={`border rounded-lg bg-white ${isActive ? "border-orange-200 bg-orange-50/30" : "border-gray-200"}`}>
            <div className="px-4 py-3 flex items-start gap-3">
              <button
                onClick={() => handlePreview(v)}
                className={`
                  flex-shrink-0 w-9 h-9 flex items-center justify-center rounded-full transition-all
                  ${isPlaying ? "bg-orange-100 text-orange-600" : "bg-gray-100 text-gray-600 hover:bg-orange-600 hover:text-white"}
                `}
                disabled={isCloning}
                title="试听"
              >
              ...
              <div className="flex items-center gap-2 flex-shrink-0 self-center">
                {isActive ? (
                  <div className="flex items-center gap-1.5 text-orange-600 bg-orange-100/50 px-2.5 py-1 rounded-full border border-orange-200/50">
                  ...
                  <button
                    ...
                    className={`
                      ...
                      ${canSelect ? "bg-white border-gray-200 text-gray-700 hover:border-orange-300 hover:text-orange-600 hover:bg-orange-50" : "bg-gray-50 border-gray-100 text-gray-400 cursor-not-allowed"}
                    `}
...
export default VoxcpmVoiceList;
