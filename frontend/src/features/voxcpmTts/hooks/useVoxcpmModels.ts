...
import type { VoxcpmTtsDownloadProvider, VoxcpmTtsDownloadTask, VoxcpmTtsModelStatus } from "../types";

const MODEL_KEY_PATTERN = /voxcpm_[0-9_]+b/i;
...
export function useVoxcpmModels() {
  const [models, setModels] = useState<VoxcpmTtsModelStatus[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await voxcpmTtsService.listModels();
      if (res?.success) {
        setModels(res.data || []);
      } else {
  ...
  const validate = useCallback(async (key: string) => {
    try {
      const res = await voxcpmTtsService.validateModel(key);
      if (!res?.success) {
  ...
  const openModelDirInExplorer = useCallback(async (key: string): Promise<void> => {
    await voxcpmTtsService.openModelDirInExplorer(key);
  }, []);

  const downloadModel = useCallback(async (key: string, provider: VoxcpmTtsDownloadProvider) => {
    ...
    try {
      const res = await voxcpmTtsService.downloadModel(key, provider);
      if (!res?.success) {
  ...
  const stopDownload = useCallback(async (key: string) => {
    ...
    try {
      const res = await voxcpmTtsService.stopDownload(key);
      if (!res?.success) {
  ...
  const loadActiveDownloads = useCallback(async () => {
    try {
      const res = await voxcpmTtsService.listDownloadTasks();
      if (!res?.success || !Array.isArray(res.data)) return;
      const map: Record<string, DownloadState> = {};
      res.data.forEach((task: VoxcpmTtsDownloadTask) => {
        if (!task?.key) return;
  ...
  const modelByKey = useMemo(() => {
    const m = new Map<string, VoxcpmTtsModelStatus>();
