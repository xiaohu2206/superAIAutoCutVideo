...
interface Qwen3DownloadProgressProps {
  download: DownloadInfo;
}

export const Qwen3DownloadProgress: React.FC<Qwen3DownloadProgressProps> = ({ download }) => {
    ...
    <div className="mt-3 bg-blue-50/50 rounded-lg p-3 border border-blue-100">
      ...
      <div className="h-1.5 w-full bg-blue-200 rounded-full overflow-hidden mb-2">
        <div 
          ...
          style={{ width: `${p}%` }}
        >
          <div className="absolute inset-0 w-full h-full animate-shimmer" style={{backgroundImage: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.3) 50%, transparent 100%)', backgroundSize: '200% 100%'}}></div>
        </div>
      </div>
