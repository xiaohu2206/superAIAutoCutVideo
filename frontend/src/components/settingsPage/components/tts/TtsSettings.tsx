...
import { TtsCredentialFormQwenOnline } from "../../components/tts/TtsCredentialFormQwenOnline";
...
export const TtsSettings: React.FC = () => {
  ...
  const currentConfigId = useMemo(
    () => getTtsConfigIdByProvider(provider),
    [provider]
  );
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const initProvider =
          cfgRes?.success && cfgRes?.data
            ? cfgRes.data?.active_config_id &&
              cfgRes.data?.configs?.[cfgRes.data.active_config_id]
  ...
  const createOrUpdateDefaultConfig = async (
    configId: string,
    prov: string
  ) => {
    try {
  ...
  const handleTestConnection = async () => {
    try {
      ...
      const proxy = (currentConfig?.extra_params || {})?.ProxyUrl as
        | string
        | undefined;
              ...
              <div className="mb-3 border border-blue-100 bg-blue-50 rounded-lg p-3 text-xs text-gray-800">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {activeVoiceDetail.gender && (
                    <LabeledChip
                      label="性别"
                      value={getGenderLabel(activeVoiceDetail.gender)}
                      variant="white"
                    />
                  )}
                  {activeVoiceDetail.typeTag && (
                    <LabeledChip
                      label="类型标签"
                      value={activeVoiceDetail.typeTag}
                      variant="white"
                    />
                  )}
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        <LabeledChip
                          label="连通性"
                          value={<span className={cls}>{statusText}</span>}
                          variant="white"
                        />
